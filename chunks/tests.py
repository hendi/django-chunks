...
class ChuckTemplateTagTestCase(BaseTestCase):
    ...
    def test_should_render_content_from_key(self):
        ...
        self.assertEqual('<div>This is the content for left box</div>', result)
    ...
    def test_should_render_empty_string_if_key_not_found(self):
        ...
        self.assertEqual('<div></div>', result)
    ...
    def test_should_cache_rendered_content(self):
        ...
        self.assertEqual('This is the content for left box', cached_result.content)


    def test_should_fail_if_wrong_number_of_arguments(self):
        with self.assertRaisesRegex(TemplateSyntaxError, "'chunk' tag should have either 2 or 3 arguments"):
            self.render_template('{% load chunks %}'
                                 '{% chunk %}')
    ...
    def test_should_fail_if_key_not_quoted(self):
        with self.assertRaisesRegex(TemplateSyntaxError, "'chunk' tag's argument should be in quotes"):
            self.render_template('{% load chunks %}'
...
class GetChuckTemplateTagTestCase(BaseTestCase):
    ...
    def test_should_get_chunk_object_given_key(self):
        ...
        self.assertEqual('<p>This is the content for left box</p>', result)
    ...
    def test_should_fail_if_wrong_number_of_arguments(self):
        with self.assertRaisesRegex(TemplateSyntaxError, 'Invalid syntax. Usage: {% get_chunk "key" as varname %}'):
            self.render_template('{% load chunks %}'
                                 '{% get_chunk %}')
    ...
    def test_should_fail_if_key_not_quoted(self):
        with self.assertRaisesRegex(TemplateSyntaxError, "Key argument to u'get_chunk' must be in quotes"):
            result = self.render_template('{% load chunks %}'
                                          '{% get_chunk home_page_left as chunk_obj %}')
